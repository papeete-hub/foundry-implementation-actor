...
The claim ADR-FIA-0005 rests on. `conformance.check` exists because a hand copy drifts; rendering
is the construction that cannot, and these are the properties that make that true rather than
merely intended — the card set is complete, it parses as an actor, it answers the definition's
doors exactly, and the one file that differs per use differs only in identity.
"""
...
from foundry_implementation_actor import cards_path, conformance, render_cards
from foundry_implementation_actor.instance import CARD_FILES
...
def test_a_second_capability_differs_only_in_identity(config, sidecar_dict, write_sidecar,
    ...
    from foundry_implementation_actor import CapabilityConfig

    other = dict(sidecar_dict,
                 capability="ZENITH.CARGO.CAP.SUP.001.MAN",
                 source_repo="zenith-lab/ZENITH.CARGO.CAP.SUP.001.MAN-implementation")
